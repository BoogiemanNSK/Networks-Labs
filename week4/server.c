...
setup_tcp_server_communication() {
    ...
    while (1) {
		...
		for (; client_data->s_name[j] != '\0'; i++, j++) {
			result.s_info[i] = client_data->s_name[j];
		}
		...
		char s_age[10];
		snprintf(s_age, 10, "%d", client_data->s_age);
		...
		for (j = 0; client_data->s_group_name[j] != '\0'; i++, j++) {
			result.s_info[i] = client_data->s_group_name[j];
		}
